...
with scores as( 
    select  exam_id,student_id, score,
            max(score) over (partition by exam_id) as hig,
            min(score) over (partition by exam_id) as low
    from    Exam
)/*Fetching the quiet students*/
select  st.student_id, st.student_name
from    student st, scores sc
where   st.student_id = sc.student_id and
        sc.low <> sc.score and sc.hig <> sc.score
MINUS/*resting the honor and disgrace students*/
select  st.student_id, st.student_name
from    student st, scores sc
where   st.student_id = sc.student_id and (
        sc.score = sc.hig or sc.score = sc.low )